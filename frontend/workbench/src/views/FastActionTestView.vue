...
const providerOptions = ['openai', 'anthropic', 'qwen', 'doubao', 'mimo', 'deepseek', 'openai_compatible']
...
const qwenBalancedProvider = computed(() => providerConfigs.value.find(item => item.id === 'qwen-balanced-service') || null)
const qwenPoolModels = computed(() => Array.isArray(qwenPoolStatus.value?.models) ? qwenPoolStatus.value.models : [])
const qwenChatModels = computed(() => qwenPoolModels.value.filter(item => item.supports_chat))
const qwenUsableModels = computed(() => qwenChatModels.value.filter(item => item.is_enabled && !item.is_exhausted))
...
const runControlBodyStyle = computed(() => ({
  maxHeight: `${Math.max(420, Math.round(PHONE_HEIGHT * phoneScale.value - 74))}px`
}))
...
function handleAttachmentChange(event) {
  ...
  for (const file of picked) {
    if (!isImageFile(file)) {
      rejected.push(`${file.name || '未知文件'} 不是图片`)
      continue
    }
    if (file.size > MAX_ATTACHMENT_SIZE) {
      rejected.push(`${file.name || '图片'} 超过 ${formatFileSize(MAX_ATTACHMENT_SIZE)}`)
      continue
    }
    if (accepted.length >= remaining) {
      rejected.push(`最多附加 ${MAX_ATTACHMENTS} 张图片`)
      continue
    }
    accepted.push(normalizeAttachment(file, accepted.length))
  }

  selectedAttachments.value = [...selectedAttachments.value, ...accepted]
  composerNotice.value = rejected[0] || (accepted.length ? '图片已作为本次测试附件加入上下文。' : '')
}

function normalizeAttachment(file, index = 0) {
  const previewUrl = URL.createObjectURL(file)
  return {
    id: `${file.name || 'image'}-${file.lastModified || Date.now()}-${Date.now()}-${index}`,
...
function removeAttachment(index) {
  ...
  if (!selectedAttachments.value.length && composerNotice.value.includes('图片已')) {
    composerNotice.value = ''
  }
}
...
async function handleVoiceRecordingStopped() {
  ...
  try {
    ...
    composerNotice.value = '语音已转成文字，可以继续补充图片附件。'
  } catch (error) {
...
function makeProviderEditor(provider = null) {
  const source = provider || defaultQwenBalancedProvider()
  return {
    originalId: source.id || '',
    id: source.id || '',
    type: source.type || 'llm',
    provider: source.provider || 'qwen',
...
function defaultQwenBalancedProvider() {
  return {
    ...
    routing: { tasks: ['planning', 'chat'], priority: 6, fallback_provider_id: null },
    credentials: { mode: 'server_secret', secret_ref: 'DASHSCOPE_API_KEY', api_key: null },
    default_headers: {},
    extra: {
...
function editQwenBalancedProvider() {
  selectProvider(qwenBalancedProvider.value || defaultQwenBalancedProvider())
}
...
async function loadSettings() {
  try {
    ...
    identityDefinitions.value = ensureArray(pickResult(results[2], []))
    knowledgeDefinitions.value = ensureArray(pickResult(results[3], []))
    qwenPoolStatus.value = pickResult(results[4], null)
...
function setupPhoneScaleObserver() {
  ...
  if (typeof ResizeObserver !== 'undefined' && previewStageRef.value) {
    previewResizeObserver = new ResizeObserver(updatePhoneScale)
        ...
        <div class="-mx-0.5 flex items-center gap-2 overflow-x-auto whitespace-nowrap px-0.5">
          ...
          <div class="flex min-w-[620px] flex-1 gap-1.5">
            ...
            <div class="min-w-[116px] flex-1 rounded-lg border border-info-200 bg-info-50 px-2.5 py-2">
              <p class="text-xs text-info-700">Qwen 可用</p>
              <p class="text-sm font-semibold text-info-700">{{ qwenUsableModels.length }}</p>
            </div>
          </div>
      ...
      <template v-else>
        <section ref="previewStageRef" class="grid grid-cols-[min-content_min-content] items-start gap-3 overflow-x-hidden 2xl:grid-cols-[min-content_min-content_minmax(320px,1fr)]">
                ...
                >
                  ...
                  >
                    ...
                    <div v-if="message.attachments?.length" class="mt-3 grid grid-cols-2 gap-2">
                      ...
                      >
                        <img v-if="attachment.previewUrl" :src="attachment.previewUrl" :alt="attachment.name" class="h-20 w-full object-cover">
                        <div
                          v-else
            ...
            <div class="border-t border-neutral-100 bg-white px-3 py-3">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <input
                  ...
                  placeholder="输入问题，也可以语音转文字或上传图片..."
                    ...
                    <div v-if="message.attachments?.length" class="mt-3 space-y-2">
                      ...
                      >
                        ...
                        <div class="min-w-0 flex-1">
                          ...
                          <p class="font-mono text-[11px] opacity-70">{{ attachment.type || 'image/*' }} · {{ formatFileSize(attachment.size) }}</p>
                        </div>
                      </div>
                    </div>
          ...
          <aside class="col-span-2 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm 2xl:col-span-1">
            ...
            <div class="space-y-4 overflow-y-auto p-5" :style="runControlBodyStyle">
              ...
              <div v-if="testResult" class="grid grid-cols-2 gap-2">
              ...
              <label class="block text-sm font-medium text-neutral-700">
                Context JSON
                <textarea v-model="contextText" rows="7" class="mt-1 w-full rounded-xl border border-neutral-200 px-3 py-2 font-mono text-sm outline-none focus:border-neutral-900 focus:ring-2 focus:ring-neutral-900/10"></textarea>
              </label>
              <label class="block text-sm font-medium text-neutral-700">
                Params JSON
                <textarea v-model="paramsText" rows="4" class="mt-1 w-full rounded-xl border border-neutral-200 px-3 py-2 font-mono text-sm outline-none focus:border-neutral-900 focus:ring-2 focus:ring-neutral-900/10"></textarea>
              </label>
              <pre v-if="testResult" class="max-h-56 overflow-auto rounded-xl bg-neutral-950 p-3 text-xs leading-5 text-neutral-100">{{ JSON.stringify(testResult, null, 2) }}</pre>
            </div>
          </aside>
        </section>
        ...
        <section v-show="!systemSettingsCollapsed" class="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <section class="min-h-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
            <div class="flex items-center justify-between gap-3 border-b border-neutral-100 px-4 py-3">
              <div class="min-w-0">
                <h3 class="text-base font-semibold text-neutral-950">千问聚合服务</h3>
                <p class="mt-0.5 truncate text-sm text-neutral-500">qwen-balanced-service · 免费模型池轮询</p>
              </div>
              <button class="shrink-0 rounded-xl bg-neutral-950 px-3 py-2 text-sm text-white hover:bg-neutral-800" @click="editQwenBalancedProvider">
                {{ qwenBalancedProvider ? '编辑' : '注册' }}
              </button>
            </div>
            <div class="max-h-[620px] overflow-y-auto p-4">
              <div class="grid grid-cols-2 gap-2">
                <div class="rounded-lg bg-neutral-50 px-3 py-2">
                  ...
                  <p class="mt-0.5 text-sm font-semibold" :class="qwenBalancedProvider ? 'text-success-700' : 'text-warning-700'">{{ qwenBalancedProvider ? '已注册' : '未注册' }}</p>
                </div>
                <div class="rounded-lg bg-neutral-50 px-3 py-2">
                  <p class="text-[11px] text-neutral-500">模型池</p>
                  <p class="mt-0.5 text-sm font-semibold text-neutral-900">{{ formatNumber(qwenPoolModels.length) }}</p>
                </div>
                <div class="rounded-lg bg-neutral-50 px-3 py-2">
                  <p class="text-[11px] text-neutral-500">聊天可路由</p>
                  <p class="mt-0.5 text-sm font-semibold text-neutral-900">{{ formatNumber(qwenChatModels.length) }}</p>
                </div>
                <div class="rounded-lg bg-neutral-50 px-3 py-2">
                  <p class="text-[11px] text-neutral-500">可用未耗尽</p>
                  <p class="mt-0.5 text-sm font-semibold text-neutral-900">{{ formatNumber(qwenUsableModels.length) }}</p>
                </div>
              </div>
              <p v-if="qwenPoolModels.length" class="mt-3 text-xs text-neutral-500">
                已加载全部 {{ formatNumber(qwenPoolModels.length) }} 个模型，表格内滚动查看。
              </p>
              <div v-if="qwenPoolModels.length" class="mt-2 max-h-[420px] overflow-auto rounded-xl border border-neutral-100">
                <table class="min-w-full text-left text-xs">
                  ...
                  <tbody class="divide-y divide-neutral-100">
                    <tr v-for="model in qwenPoolModels" :key="model.model">
